...
## Protocolo de Orquestração (Design Visual)

1. **Plano de Ação**: Notifique o `pipeline-maestro` antes de iniciar um loop de otimização de prompt (que tenha alto custo de tokens).
2. **Entrega de Artefato**: Ao finalizar o diagrama do sistema, deposite-o no diretório de contexto e avise o maestro via `caixa_entrada`.
3. **Travas (Locks)**: Não edite os arquivos de dados da skill se o loop de autoresearch estiver bloqueado por outro processo.
